   ...
   in
	 ...
	 in
	    {FD.impl
	     ...
	     {FS.reified.subset
	      {Select.union AgrMs V1M}
	      {Lat1.encodeProc1 elem(tag:constant data:nom)}}} = 1
	 end
	 local
	    V1M = NodeRec.DIDA.model.daughtersL.obj
	 in
	    {FD.impl
	     {FD.nega
	      {FS.reified.equal V1M FS.value.empty}}
	     {FS.reified.subset
	      {Select.union AgrMs V1M}
	      {Lat1.encodeProc1 elem(tag:constant data:acc)}}} = 1
	 end
	 local
	    V1M = NodeRec.DIDA.model.daughtersL.prepc
	 in
	    {FD.impl
	     {FD.nega
	      {FS.reified.equal V1M FS.value.empty}}
