        ...
        fun {$ UsrV1NodeRec}
           local Auto1I = {ByNeed fun {$}
               {PW.'in'
                ...
                {Lat1.encodeProc1 elem(tag:constant data:gen)}}
             end}
           in
            {PW.eq
             {FS.intersect
              {FS.intersect
               {FS.intersect
                ...
                {PW.ifThenElseSet
                 {PW.'in'
                  UsrV1NodeRec.{Principle.dVA2DIDA UsrD}.attrs.'agr'
                  {Lat1.encodeProc1 elem(tag:constant data:dat)}}
                 NodeRecs.1.nodeSet
                 {FS.diff
                  NodeRecs.1.nodeSet
                  UsrV1NodeRec.{Principle.dVA2DIDA UsrD}.model.mothersL.'iobj'}}}
               {PW.ifThenElseSet
                Auto1I
                NodeRecs.1.nodeSet
                {FS.diff
                 NodeRecs.1.nodeSet
                 UsrV1NodeRec.{Principle.dVA2DIDA UsrD}.model.mothersL.'genobj'}}}
              {PW.ifThenElseSet
               Auto1I
               NodeRecs.1.nodeSet
               {FS.diff
                NodeRecs.1.nodeSet
                UsrV1NodeRec.{Principle.dVA2DIDA UsrD}.model.mothersL.'genmod'}}}
             NodeRecs.1.nodeSet}
           end %local
        end}
       = 1
